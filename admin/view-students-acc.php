<?php
    ...
    if(isset($_GET['del'])) {
        ...
        $adn3="SELECT room from userregistration where id=$id";
        echo "<script>alert($id);</script>" ;
        $stmt2= $mysqli->prepare($adn3);
        $stmt2->execute();
        ...
        while($row=$res->fetch_object())
        {
         ...
         $adn2="UPDATE rooms set occupiedbed=occupiedbed-1 where room_no=$delid";

            $stmt= $mysqli->prepare($adn);
            $stmt->bind_param('i',$id);
            $stmt->execute();
            $stmt->close();	
            $stmt1= $mysqli->prepare($adn2);
            $stmt1->execute();
